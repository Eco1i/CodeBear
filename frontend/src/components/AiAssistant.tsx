...
const AI_SCOPE_STORAGE_KEY = "maxiong.ai.scope-type";
const AI_ACTIVE_CONVERSATION_STORAGE_KEY = "maxiong.ai.active-conversation";

function readStoredScopeType(): AiScopeType {
  try {
...
function storeActiveConversationId(conversationId: string | null): void {
  try {
...
interface ConversationMessage extends AiHistoryMessage {
  id: string;
...
function storedConversationMessage(item: StoredConversationMessage): ConversationMessage {
  return {
    id: item.id,
    role: item.role,
...
type ConversationDateGroup = "今天" | "昨天" | "更早";

function conversationDateGroup(value: string): ConversationDateGroup {
...
function scopesMatch(left: AiScope, right: AiScope): boolean {
  return (
...
function restoredScopeFromConversation(
...
export function AiAssistant({
  ...
  useEffect(() => {
    ...
    api.aiConversations()
      .then(async (items) => {
        ...
        const detail = await api.aiConversation(initialConversation.id);
        if (cancelled || selectionRevision !== conversationSelectionRevisionRef.current) return;
  ...
  useEffect(() => {
    ...
    setTablePeekError("");
    setTablePeekDetail(null);
    api.table(tablePeekEvidence.table_id, controller.signal)
      .then((nextDetail) => {
  ...
  const reloadConversationHistory = async () => {
    ...
    setHistoryError("");
    try {
      setConversations(await api.aiConversations());
    } catch (error) {
      setHistoryError(errorText(error));
    } finally {
      setHistoryLoading(false);
  ...
  const renameConversation = async (conversationId: string, title: string) => {
    setHistoryBusyId(conversationId);
    try {
      rememberConversation(await api.renameAiConversation(conversationId, title));
    } catch (error) {
      notice.error(errorText(error));
      throw error;
    } finally {
  ...
  const saveSettings = async () => {
    ...
    try {
      await api.testAi(normalizedKey);
  ...
  const clearKey = () => {
    modal.confirm({
      ...
      cancelText: "取消",
      onOk: async () => {
        const nextSettings = await api.clearAiKey();
        setSettings(nextSettings);
        setAssistantName(nextSettings.assistant_name);
        setAssistantAccessory(nextSettings.assistant_accessory);
        setApiKey("");
  ...
  const sendQuestion = async (value = question) => {
    ...
    try {
      if (conversationId) {
        const stored = await api.appendAiConversationMessage(
          conversationId,
          conversationMessageInput(userMessage, scopeOption),
        );
        rememberConversation(stored.conversation);
      } else {
        const created = await api.createAiConversation(conversationMessageInput(userMessage, scopeOption));
      ...
      const result: AiChatResponse = await api.aiChat(
        normalized,
        scope,
        history,
        controller.signal,
