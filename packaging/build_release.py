...
"""
    ...
    shutil.copy2(
        ROOT / "frontend" / "node_modules" / "@fontsource" / "jetbrains-mono" / "LICENSE",
        license_dir / "JetBrains-Mono-OFL.txt",
    )
    shutil.copy2(
        ROOT / "frontend" / "node_modules" / "@fontsource-variable" / "noto-sans-sc" / "LICENSE",
        license_dir / "Noto-Sans-SC-OFL.txt",
    )
    license_script = r"""
...
names = (
    "fastapi", "uvicorn", "lxml", "pydantic", "pydantic-core",
    "python-multipart", "pyinstaller", "pyinstaller-hooks-contrib",
    "pystray", "pillow",
)
entries = []
for name in names:
    ...
    for relative_file in distribution.files or ():
        if not any(
            ...
            continue
        source = distribution.locate_file(relative_file)
        if source.is_file():
            entries.append({
                "name": name,
...
"""
    result = subprocess.run(
        [str(build_python), "-c", license_script],
        check=True,
        text=True,
        encoding="utf-8",
        stdout=subprocess.PIPE,
    ...
    for entry in json.loads(result.stdout):
        ...
        safe_name = "-".join((entry["name"], entry["version"], *entry["parts"]))
        shutil.copy2(source, license_dir / safe_name)
