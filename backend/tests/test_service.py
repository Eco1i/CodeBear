...
VIEW_SAMPLE_PDM = """<?xml version="1.0" encoding="UTF-8"?>
<?PowerDesigner Name="视图测试模型" Target="ORACLE Version 11g" version="15.1.0.2850"?>
<Model xmlns:a="attribute" xmlns:c="collection" xmlns:o="object">
  <c:Tables>
    <o:Table Id="o10">
      ...
      <c:Columns>
        <o:Column Id="o11"><a:Name>订单编号</a:Name><a:Code>ORDER_ID</a:Code><a:DataType>NUMBER(16)</a:DataType></o:Column>
      </c:Columns>
      <c:Keys><o:Key Id="o12"><c:Key.Columns><o:Column Ref="o11"/></c:Key.Columns></o:Key></c:Keys>
      <c:PrimaryKey><o:Key Ref="o12"/></c:PrimaryKey>
    </o:Table>
  </c:Tables>
  <c:Views>
    <o:View Id="o30">
      <a:Name>活跃委托视图</a:Name>
      <a:Code>V_ACTIVE_ORDER</a:Code>
      <a:Comment>演示视图</a:Comment>
      <c:Columns>
        <o:Column Id="o31"><a:Name>订单编号</a:Name><a:Code>ORDER_ID</a:Code><a:DataType>NUMBER(16)</a:DataType></o:Column>
        <o:Column Id="o32"><a:Name>订单金额</a:Name><a:Code>ORDER_AMOUNT</a:Code><a:DataType>NUMBER(18,2)</a:DataType></o:Column>
      </c:Columns>
    </o:View>
  </c:Views>
</Model>
...
def test_views_are_indexed_searchable_and_readonly(tmp_path: Path) -> None:
    service = make_service(tmp_path)
    source = tmp_path / "视图样本.pdm"
    source.write_text(VIEW_SAMPLE_PDM, encoding="utf-8", newline="\n")
    project = service.create_project("视图测试")
    result = service.import_staged_files(str(project["id"]), "", [(source.name, source)], overwrite=False)
    assert result["errors"] == []

    search = service.search_tables(
        project_id=str(project["id"]),
        scope_type="project",
        scope_path="",
        mode="table",
    ...
    view_item = next(item for item in search["items"] if item["kind"] == "view")
    assert view_item["code"] == "V_ACTIVE_ORDER"

    detail = service.table_detail(str(view_item["id"]))
    assert detail["kind"] == "view"
    assert len(detail["fields"]) == 2

    with pytest.raises(ServiceError) as exc_info:
        service.save_table_fields(str(view_item["id"]), str(detail["source_hash"]), [], None)
    assert exc_info.value.code == "view_readonly"

    catalog = service.ddl_catalog(str(project["id"]), include_tables=True)
    assert catalog["table_count"] == 1

    with pytest.raises(ServiceError) as exc_info:
        service.generate_ddl([str(view_item["id"])], {"target_db": "mysql"})
    assert exc_info.value.code == "ddl_view_not_supported"
